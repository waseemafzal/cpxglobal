...
font-weight: bold;}  
       ...
       <?php   
            $obj = $this->session->userdata('payout_paymentresponse_refund');
			...
			{ 
				 ...
				 $aStatus = array();
				  for($index = 0 ; $index < sizeof($obj->items); $index++)
					...
					/*************status phone ******************/
					if(is_array($aStatus) AND count($aStatus) > 0)
					{
						for($index = 0 ; $index  < count($aStatus) ; $index ++)
						{
						
							if($aStatus[$index] ==PAID)
							{
								$to = $this->session->userdata('payout_paymentresponse_email');
								$content =  'Your have been refunded amount of  $ '.$this->session->userdata('payout_paymentresponse_amount').'   sent to you Paypal Account.';
							...
							if($aStatus[$index] ==CANCELLEDPAYPAL)
							{
								...
								$to = $this->session->userdata('payout_paymentresponse_email');
								$content = ' Your refunding process have been cancel with amount of  $  '.$this->session->userdata('payout_paymentresponse_amount').'  Please Contact you Skillsqaurd Support Team.';
      ...
      <div  class="box" style="padding: 0px 0px 0px 30px;">
        ...
        <p></p>
				 ...
				 $this->session->set_userdata('payout_paymentresponse_refund', '');
				 $this->session->set_userdata('payout_paymentresponse_email', '');
				 $this->session->set_userdata('payout_paymentresponse_amount', '');
