		 ...
		 public function  getsavedlectures()
	        {
				...
				$data =   $this->db->query(
				"SELECT cl.title as course_lecture_title,cld.title as lecture_video_title,cld.duration as lecture_video_duration,cld.video as lecture_video,
					...
					$courselecturelist .='<ul class="accordion">';
					 ...
					 foreach($alist as $coursetitle =>$coursedetail):
						...
						$courselecturelist .='<li>';
							$courselecturelist .='<a class="toggle" href="javascript:void(0);"><strong>'.$coursetitle.'</strong></a>';
							...
							for($index = 0 ; $index < count($coursedetail['title']); $index ++):
								...
								$courselecturelist .='<li style="margin-left:5px;color:#000;" id="list_'.$id.'">
								   <a href="javascript:void(0)">'.$coursedetail['title'][$index].'</a>&nbsp;
								    <a href="javascript:void(0);"  onclick="removelecture('.$id.')" style="color:red;"><strong>X</strong></a>
									<span class="pull-right">
										<a href="javascript:void(0);"  onclick="preview_video(\'' . $id . '\')" title="Play"> <span class="fa  fa-play-circle"></span></a></span>
