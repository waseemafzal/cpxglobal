...
<section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li > <a href="product/add/" class="btn btn-sm btn-su">Add Package</a></li>
      </ol>
    </section>
    <!-- Main content -->
	...
	foreach ($data->result() as $row){
		...
		*/
        ...
        <td class="center"><?php echo html_cut($row->product_description, 100) ?></td>
        <td>
		<?php
        $groups =  getProductGroupTitle($row->id);
		if($groups!=0)
        foreach($groups->result() as $group){
        ?>
        <label class="label label-success"><?php echo $group->title ?></label>
        <?php
        }
        ?>
        </td>
                 <td class="center"><?php echo $row->package_type ?></td>
