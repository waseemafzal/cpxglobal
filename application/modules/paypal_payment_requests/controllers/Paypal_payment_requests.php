...
class Paypal_payment_requests extends MX_Controller {
...
/*
		$aData['data'] =$this->db->query('SELECT o.*,pst.statusTitle as status,pst.status_id,u.name,u.phone,o.status FROM `payment_requests` as o 

		join payment_status_titles as pst on pst.status_id=o.status

join users as u on u.id=o.user_id ORDER BY o.status ASC ');*/

	$aData['data'] = $this->db->query("SELECT PR.*,
		(SELECT username FROM freelancers AS F  WHERE  F.user_id =PR.user_id ) AS name,
		(SELECT U.phone FROM users AS U WHERE  U.id =PR.user_id ) AS phone
