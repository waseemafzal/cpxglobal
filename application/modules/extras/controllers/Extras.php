...
class Extras extends MX_Controller {
	...
	public $view = "view";
	public $tbl = 'extras_group';
	public $tbl_extra = 'extras';
	...
	$aData['group_id'] =$_POST['id'];
		$this->load->view('extras/view_items',$aData);
	}
	...
	public function removeExtras(){ 
		extract($_POST);
		$this->db->where(array('group_id'=>$group_id,'product_id'=>$product_id));
		$result= $this->db->delete('product_extra_group');
		switch($result){
			case 1:
			$arr = array('status' => 1,'message' => "Deleted Succefully !");
			echo json_encode($arr);
	...
	function saveExtra(){ 
		...
		if ($this->form_validation->run()==false){
			...
			$result = $this->crud->saveRecord($PrimaryID,$_POST,$this->tbl_extra);
