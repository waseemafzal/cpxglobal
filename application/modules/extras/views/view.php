...
<section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li > <a href="extras/add" class="btn btn-sm btn-su">Add </a></li>
      </ol>
    </section>
    <!-- Main content -->
		...
		*/
        ...
        <td><?php echo $row->title;?></td>
            ...
            <td class="center">
                                   ...
                                   <a id="anchor_<?PHP echo $row->id;?>" href="javascript:void(0);"  
                                       onclick="changeStatus('<?PHP echo $row->id;?>','<?PHP echo $row->status;?>','extras_group');" >
        ...
        <td class="center">
		 <a href="javascript:void(0)" id="<?php echo$row->id;?>"   data-toggle="tooltip" title="view Images " class="btn  btn-warning slider-detail btn_action" >
             <?php echo ucwords(this_lang('Extra Items ')); ?>
              <i class="fa fa<?php echo$row->id;?> fa-plus"></i>
            </a> 
            <a data-toggle="tooltip" title=" <?php echo ucwords(this_lang('Edit'));?>" class="btn btn-info " href="extras/edit/<?php echo $row->id;?>">
                <i class="glyphicon glyphicon-edit icon-white"></i>
                Edit
            </a>
            <a data-toggle="tooltip" title=" <?php echo ucwords(this_lang('Delete'));?>" class="btn btn-danger " href="javascript:void(0)" onClick="deleteRecord('<?php echo$row->id;?>','extras_group');">
		   ...
		   if (row.child.isShown()) {
            ...
            $('.slider-detail .fa'+id).addClass('fa-plus');
			  ...
			  $('.slider-detail  .fa'+id).addClass('fa-minus');
			  				  $.ajax({
			url : "<?php echo base_url().'extras/getItems'; ?>",
			type: 'POST',
			data: {id: id},
		beforeSend: function() {
