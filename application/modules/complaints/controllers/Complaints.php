...
class Complaints extends MX_Controller {
  ...
  public function getComplaintsDetail()
  {
	...
	if($query->num_rows()>0)
	{
		$status = 1;
		$user = $query->row();
		$other_address = json_decode($user->other_address);
		

	    $userHtml.='<div id="inner">';
		$userHtml.='<p style="margin: 0;"   align="center"><b style="font-size:17px;">Other Details</b></p>';
		$userHtml.='<div style="margin: 0;"> <strong>Address:</strong> '.$other_address->address.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>City:</strong> '.$other_address->city.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>State:</strong> '.$other_address->state.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>Zipcode:</strong> '.$other_address->zipcode.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>Product Service Involved:</strong> '.$other_address->product_service_involved.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>DOB:</strong> '.$other_address->dob.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>Contacted:</strong> '.$other_address->contacted.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>Think:</strong> '.$other_address->think.'</div>';
		$userHtml.='<div style="margin: 0;"> <strong>Summary:</strong> '.$other_address->summary.'</div>';
		$html.=$userHtml;
