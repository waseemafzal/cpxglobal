...

     {
	   ...
	   if(isset($filterdata['statusfilter'])  AND !empty($filterdata['statusfilter']) AND $filterdata['statusfilter']!='-1')
		...
		if(isset($filterdata['shiftfiltervar'])  AND !empty($filterdata['shiftfiltervar']) AND $filterdata['shiftfiltervar']!='-1')
		{
			...
			$querycount = $this->db->query("SELECT count(TS.id) as total_rows_count FROM `" . $this->tbl_services . "` AS TS INNER JOIN `". $this->users . "` AS U ON U.id = TS.user_id  INNER JOIN `".$this->freelancers."`  AS F ON F.user_id = TS.user_id  WHERE TS.status = ".$this->active."  AND TS.id  ".$sellerservicesidsstr."
			  ...
			  ");
				...
				{
					/*echo 'count'.$querycount->num_rows();
					echo '</br>';*/
					//echo count($querycount->result());
           ...
           $query = $this->db->query("
					...
					(SELECT TCR.currency_code FROM `".$this->tbl_currency."`  AS TCR WHERE TCR.currency_id = U.currency_id ) AS currency_unit,TS.price AS price ,TSM.media,(SELECT CONCAT( ROUND (sum(rating)/count(rating),1),'=>',count(service_id))    FROM `" . $this->tbl_review_rating . "` WHERE TS.id = tbl_review_rating.service_id) AS  service_rating FROM `" . $this->tbl_services . "` AS TS INNER JOIN `" . $this->tbl_services_media . "` AS TSM  ON TS.id  = TSM.service_id  INNER JOIN `" . $this->users . "` AS U  ON U.id  = TS.user_id  INNER JOIN `" . $this->freelancers . "` AS F  ON F.user_id  = TS.user_id
WHERE TS.status = ".$this->active."   ".$sellerservicesidsstr." "  
		...
		if (count($query->result()) > 0) {
			...
			foreach ($query->result() as $row) 
			{
				...
				else
				{
					...
					$html .= ' <a href="javascript:void(0);"  data-target="#signup" data-toggle="modal"><i class="fa fa-heart"></i></a>';
		...
		//echo 'time-->'. round($time_elapsed_secs = microtime(true) - $start,3);
		//die();
		}
		else
		{
...
WHERE status = " . $this->active . " AND TS.user_id = '" . $userid . "'");
...

    {
        ...
        $query      = $this->db->query("SELECT TS.*,
                ...
                (SELECT media FROM  " . $this->tbl_services_media . "  AS TSM WHERE TSM.service_id = TS.id) AS service_image
