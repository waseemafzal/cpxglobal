   ...
   /**===============wishlist====================**/
    ...
    /**===============services====================**/
...

     {
		...
		{
			if(!empty($filterdata['alititude']))
			{
			  $aalititude =  explode('_',$filterdata['alititude']);	
			  $latitude = $aalititude[0];
			  $longitude = $aalititude[1];
	   ...
	   if(isset($filterdata['statusfilter'])  AND !empty($filterdata['statusfilter']) AND $filterdata['statusfilter']!='-1')
		...
		if(isset($filterdata['shiftfiltervar'])  AND !empty($filterdata['shiftfiltervar']) AND $filterdata['shiftfiltervar']!='-1')
		{
            ...
            if($filterserachvar==false)
			{
			  
				$querycount = $this->db->query("SELECT count(TS.id) as total_rows_count FROM `".$this->tbl_services."` AS TS WHERE TS.status = ".$this->active."   ");
