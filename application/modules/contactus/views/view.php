...
<section class="content-header">
	...
	foreach ($data->result() as $row){
        ...
        <td class="center"><?php echo $row->message; ?></td>
            <td>
                <a  class="btn btn-info orderDetail btn-xs" onclick="getContactusDetail('<?PHP echo $row->id;?>')">
                    <i class="fa fa-eye"></i>
                    View Detail
                </a>
           </td>
...
function getInfo(id){
 $.ajax({
  ...
  function getContactusDetail(id)
  {
	$('#ProductModal').modal('show');
	$('#ProductDetaBody').html('<i class="fa fa-cog fa-spin" style="font-size:24px"></i>');
	
	  $.ajax({
		url: "<?php echo base_url().'Contactus/getContactusDetail'; ?>",
		type: 'POST',
		data: {id:id},
		dataType : "json",
        success: function(response) 
